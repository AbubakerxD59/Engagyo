    ...
    <script>
        $(document).ready(function() {
                    ...
                    function renderEngagementsChart(insights, comp, selectedMetricKey, platform) {
                        ...
                        selectedMetricKey = selectedMetricKey || (platform === 'pinterest' ? 'reach' : (platform ===
                            'tiktok' ? 'view_count' : 'engagements'));
                        var opt = opts.find(function(o) {
                            return o.key === selectedMetricKey;
                        }) || (platform === 'pinterest' ? opts[0] : (platform === 'tiktok' ? chartMetricOptionsTiktok[
                            0] : chartMetricOptions[3]));
                    ...
                    function initEngagementsChart(insights, metricKey, platform) {
                        platform = platform || 'facebook';
                        var opts = chartMetricOptionsForPlatform(platform);
                        metricKey = metricKey || (platform === 'pinterest' ? 'reach' : (platform === 'tiktok' ?
                    ...
                    function hasMeaningfulInsights(insights, platform) {
                        ...
                        if (platform === 'pinterest') {
                            ...
                            if (insights.video_views_by_day && Object.keys(insights.video_views_by_day).length > 0)
                            return true;
                            return false;
                        }
                        if (platform === 'tiktok') {
                    ...
                    var postInsightDisplayOrderPinterest = ['post_impressions', 'pin_saves', 'outbound_clicks',
                        'pin_clicks', 'video_mrc_view', 'total_comments', 'total_reactions'
                    ];
                    ...
                    var postInsightDisplayOrderInstagram = [
                        ...
                        'post_saves', 'post_shares', 'post_engagement_rate'
                    ];
                    ...
                    function renderPostsList(posts, since, until, searchQuery, sortBy, sortOrder, platform, totalPosts) {
                        ...
                        if (!posts || posts.length === 0) {
                            if (currentPostsFetching) {
                                var fetchingMsg = currentPostsFetchingMessage ||
                                    'Posts for this page are being fetched. Please check back shortly.';
                                return '<div class="analytics-posts-placeholder text-center py-5">' +
                        ...
                        var searchBar =
                            ...
                            '</div></div>';
                        var sortLabel = (sortOptionsList.find(function(o) {
                            return o.key === sortBy;
                        }) || sortOptionsList[0]).label;
                        var sortDropdown = '<div class="analytics-posts-sort-wrap">' +
                            ...
                            sortOptionsList.map(function(o) {
                                return '<a class="dropdown-item" href="#" data-sort="' + o.key + '">' + (o.key ===
                                        sortBy ? '<i class="fas fa-check mr-2 text-primary"></i>' :
                                        '<span class="mr-2" style="width:1em;display:inline-block;"></span>') +
                                    escapeHtml(o.label) + '</a>';
                            }).join('') + '</div></div></div>' +
                            '<div class="analytics-posts-sort-group">' +
                            '<label class="analytics-posts-sort-label">Order</label>' +
                            '<div class="btn-group btn-group-sm" role="group">' +
                            '<button type="button" class="btn btn-sm ' + (sortOrder === 'desc' ? 'btn-primary' :
                                'btn-outline-secondary') +
                            ' analytics-posts-order-btn" data-order="desc" title="Descending"><i class="fas fa-sort-amount-down"></i></button>' +
                            '<button type="button" class="btn btn-sm ' + (sortOrder === 'asc' ? 'btn-primary' :
                                'btn-outline-secondary') +
                            ' analytics-posts-order-btn" data-order="asc" title="Ascending"><i class="fas fa-sort-amount-up"></i></button>' +
                            '</div></div></div>';
                        var html = '<div class="analytics-posts-tab-content">' +
                            '<div class="analytics-posts-header mb-3">' +
                            '<div class="analytics-posts-header-left">' +
                            '<div class="analytics-posts-title mb-2"><i class="fas fa-newspaper mr-2"></i>POSTS (' +
                            totalPosts +
                        ...
                        filtered.forEach(function(post) {
                            ...
                            var createdHtml = created.time ?
                                '<span class="analytics-post-time">' + escapeHtml(created.time) +
                                '</span><span class="analytics-post-day">' + escapeHtml(created.date) + '</span>' :
                                '<span class="analytics-post-day">-</span>';
                            var postMediaType = (post.media_type || '').toString().toLowerCase();
                            var isThreadsVideo = platform === 'threads' && (postMediaType === 'video' || (post
                                .type || '').toString().toLowerCase() === 'video');
                            var img = '';
                            if (isThreadsVideo && post.full_picture) {
                                var vSrc = escapeHtml(post.full_picture);
                                img =
                                    '<video class="analytics-post-thumb" preload="metadata" playsinline controls>' +
                    ...
                    function renderPageInsights(insights, pageName, duration, since, until, pagePosts, platform) {
                        ...
                        if (!hasMeaningfulInsights(insights, platform)) {
                            ...
                            var cards = platform === 'pinterest' ? [
                                ...
                                ['followers', 'Followers', false],
                                ['reach', 'Reach', false],
                                ['video_views', 'Profile Views', false],
                                ['engagements', 'Interactions', false]
                            ] : [
                                ['followers', 'Followers', false],
                                ['reach', 'Reach', false],
                                ['video_views', 'Video Views', false],
                            ...
                            if (platform === 'facebook') {
                                note = '<p class="small mb-3" style="color: #856404;"><i class="fas fa-info-circle mr-1"></i>Page Insights data is only available on Pages with 100 or more likes.</p>';
                            } else if (platform === 'pinterest') {
                                note = '<p class="small mb-3 text-muted"><i class="fas fa-info-circle mr-1"></i>Pinterest board metrics aggregate pin analytics (impressions, saves, clicks) for pins on this board. Pinterest limits analytics history (typically up to ~90 days).</p>';
                            } else if (platform === 'tiktok') {
                                note = '<p class="small mb-3 text-muted"><i class="fas fa-info-circle mr-1"></i>Account metrics from TikTok user.info.stats (followers, total likes, and public video count).</p>';
                            } else if (platform === 'instagram') {
                                note = '<p class="small mb-3 text-muted"><i class="fas fa-info-circle mr-1"></i>Instagram account metrics from Meta user insights (reach, followers, profile views, interactions). Post metrics come from media insights.</p>';
                            } else {
                                note = '<p class="small mb-3 text-muted"><i class="fas fa-info-circle mr-1"></i>Threads insights are aggregated from available media metrics for the selected date range.</p>';
                            }
                            ...
                            analyticsRequest = $.get(analyticsUrl, params)
                                ...
                                .done(function(res) {
                                    ...
                                    if (res.selectedPage) {
                                        ...
                                        html += renderPageInsights(res.pageInsights, res.selectedPage.name,
                                            currentDuration,
                                    ...
                                    if (res.selectedPage && res.pageInsights) {
                                        ...
                                        var byDayKeyResolved = optRow ? optRow.byDayKey :
                                            (plt === 'pinterest' ? 'reach_by_day' : (plt === 'tiktok' ?
                                                'view_count_by_day' : 'engagements_by_day'));
                        ...
                        function bindChartMetricHandlers() {
                            ...
                            $(document).on('click', '.chart-metric-option', function(e) {
                                ...
                                var plt = (typeof currentPlatform !== 'undefined' && currentPlatform) ?
                        ...
                        function refreshPostsTab() {
                            ...
                            var query = $('#analyticsPostsSearch').length ? $('#analyticsPostsSearch').val().trim() :
                                currentPostsSearchQuery;
                            currentPostsSearchQuery = query;
                            var postsContent = renderPostsList(posts, currentSince, currentUntil, query, currentPostsSortBy,
                                currentPostsSortOrder, currentPlatform, currentPostsTotal);
                        ...
                        function bindPostsSortHandler() {
                            $content.off('click',
                                '.analytics-posts-sort-btn, .dropdown-item[data-sort], .analytics-posts-order-btn');
                        ...
                        function bindAnalyticsPostsInfiniteScroll() {
                            ...
                            $(window).on('scroll.analyticsPostsLoadMore', function() {
                                ...
                                if (scrollBottom >= triggerPoint && bindVersion ===
                                    analyticsPostsScrollBindVersion) {
