                                            ...
                                            <div class="card-body">
                                                @if ($package)
                                                    <div class="text-center py-4">
                                                        ...
                                                        <div class="mb-3">
                                                            <span
                                                                class="badge badge-pill badge-{{ $packageStatus === 'Active' ? 'success' : ($packageStatus === 'Expired' ? 'danger' : ($packageStatus === 'Lifetime' ? 'info' : 'secondary')) }} px-3 py-2">
                                                                {{ $packageStatus }}
                                                            </span>
                                                        </div>
                                                            ...
                                                            @if ($expiryDate && $packageStatus !== 'Lifetime')
                                                                <div class="d-flex justify-content-between mb-2">
                                                                    <span class="text-muted"><i
                                                                            class="far fa-calendar-alt mr-2"></i>Expires
                                                                        On:</span>
                                                                    <span
                                                                        class="font-weight-bold">{{ $expiryDate->format('M j, Y') }}</span>
                                                                </div>
                                                                <div class="d-flex justify-content-between">
                                                                    <span class="text-muted"><i
                                                                            class="far fa-clock mr-2"></i>Days Left:</span>
                                                                    <span
                                                                        class="font-weight-bold {{ $expiryDate->diffInDays(now()) < 5 ? 'text-danger' : 'text-success' }}">
                                                                        {{ max(0, ceil(now()->floatDiffInDays($expiryDate, false))) }}
                                                                        Days
                                                                    </span>
                                                                </div>
                                                ...
                                                @else
                                                    <div class="text-center py-5">
                                                        ...
                                                        <p class="small text-muted mb-4">You are currently on the free tier
                                                            or have no active subscription.</p>
                                                        <button class="btn btn-primary btn-sm"
                                                            onclick="$('#upgrade-tab').tab('show')">
                                                            View Plans
                                                        </button>
                                                    </div>
                                                @endif
                                                    ...
                                                    <div class="row">
                                                        @foreach ($featuresWithUsage as $feature)
                                                            <div class="col-md-6 mb-4">
                                                                <div
                                                                    class="p-3 rounded bg-light h-100 border-left-primary position-relative overflow-hidden feature-card">
                                                                    <div
                                                                    ...
                                                                    @if (!$feature['is_unlimited'] && $feature['limit'] !== null)
                                                                        <div
                                                                            ...
                                                                            <span>{{ number_format($feature['usage']) }}
                                                                                used</span>
                                                                            <span>{{ number_format($feature['limit']) }}
                                                                                limit</span>
                                                                        </div>
                                                                        ...
                                                                        @if ($feature['remaining'] !== null && $feature['remaining'] <= $feature['limit'] * 0.1)
                                                                            <small class="text-danger mt-1 d-block">
                                                                                <i
                                                                                    class="fas fa-exclamation-circle mr-1"></i>Running
                                                                                low!
                                                                            </small>
                                                                        @endif
                                                                    @else
                            ...
                            <div class="tab-pane fade" id="upgrade" role="tabpanel" aria-labelledby="upgrade-tab">
                                <div class="row justify-content-center">
                                    <div class="col-12 text-center mb-5">
                                        ...
                                        <p class="text-muted" style="font-size: 1.1rem;">Unlock more features and power up
                                            your workflow</p>
                                    </div>
                                </div>
                                <div class="row justify-content-center">
                                    @foreach ($packages as $pkg)
                                        ...
                                        <div class="col-md-6 col-lg-4 mb-4">
                                            <div
                                                class="card h-100 pricing-card border-0 shadow-sm {{ $isCurrentInfo ? 'current-plan-card ring-primary' : '' }}">
                                                @if ($isCurrentInfo)
                                                ...
                                                <div class="card-body p-4 d-flex flex-column">
                                                    ...
                                                    <div class="pricing-price text-center mb-4">
                                                        ...
                                                        <span
                                                            class="amount display-4 font-weight-bold text-dark">{{ number_format($pkg->price, 2) }}</span>
                                                        @if (!$isLifetime)
                                                            <span class="period text-muted">/ {{ $pkg->duration }}
                                                                {{ Str::plural($pkg->date_type, $pkg->duration) }}</span>
                                                        @else
                                                    ...
                                                    <div class="plan-features mb-4 flex-grow-1">
                                                        <ul class="list-unstyled">
                                                            @foreach ($pkg->features as $feature)
                                                                <li
                                                                    class="mb-3 d-flex align-items-start {{ !$feature->pivot->is_enabled ? 'text-muted' : '' }}">
                                                                    @if ($feature->pivot->is_enabled)
                                                                        <i
                                                                            class="fas fa-check-circle text-success mt-1 mr-2"></i>
                                                                    @else
                                                                        <i
                                                                            class="fas fa-times-circle text-muted mt-1 mr-2"></i>
                                                                    @endif
                                                                    <span style="font-size: 0.95rem;">
                                                                        {{ $feature->name }}
                                                                        @if ($feature->pivot->is_unlimited)
                                                                            <span
                                                                                class="badge badge-light border ml-1">Unlimited</span>
                                                                        @elseif($feature->pivot->limit_value > 0)
                                                                            <span
                                                                                class="badge badge-light border ml-1">{{ $feature->pivot->limit_value }}</span>
                                                                        @endif
                                                                    </span>
                                                                </li>
                                                            @endforeach
                                                    ...
                                                    <div class="text-center mt-auto">
                                                        @if ($isCurrentInfo)
                                                            ...
                                                            @else
                                                                <a href="{{ route('payment.checkout', $pkg->id) }}"
                                                                    class="btn btn-info btn-block btn-lg shadow-sm hover-lift">
                                                                    Renew Plan
                                                                </a>
                                                            @endif
                                                        @else
                                                            <a href="{{ route('payment.checkout', $pkg->id) }}"
                                                                class="btn btn-primary btn-block btn-lg shadow-sm hover-lift">
                                                                Choose Plan
                                                            </a>
                                                        @endif
                                                    </div>
