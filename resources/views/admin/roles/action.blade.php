@if (!in_array($role->name, ['User']))
    <div class="d-flex">
        ...
        @if (!in_array($role->name, ['Super Admin', 'Admin', 'Staff']))
            @can('edit_role')
                <div>
                    <a href="{{ route('admin.roles.edit', $role->id) }}" class="btn btn-outline-primary btn-sm">Edit</a>
                </div>
            @endcan
            @can('delete_role')
                <div>
                    <button class="btn btn-outline-danger btn-sm delete-btn" onclick="confirmDelete(event)">Delete</button>
                    <form action="{{ route('admin.roles.destroy', $role->id) }}" method="POST" class="delete_form">
                        @csrf
                        @method('DELETE')
                    </form>
                </div>
